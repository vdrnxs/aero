...
2. Compile with `cd foundry && forge build` → generates `foundry/out/*.json`
3. Run `pnpm sync-abis` → copies ABIs to `apps/web/lib/contracts/`
4. Import ABIs in frontend: `import SimpleTokenABI from '@/lib/contracts/SimpleToken.json'`
5. Use with Wagmi v2 hooks:
   ```typescript
...
### Example Contract: SimpleToken

The starter includes `SimpleToken.sol` - a basic ERC20-like token:
- **Name**: "Simple Token" (SIM)
- **Supply**: 1,000,000 tokens (18 decimals)
- **Functions**: `transfer()`, `approve()`, `transferFrom()`, `balanceOf()`
- **Deploy**: Deployer receives all initial supply
- **Demo**: Tokens appear in MetaMask after importing contract address
...
- Example SimpleToken contract (ERC20-like, 1M supply)
- ABI sync pipeline
- Web3 integration with Wagmi v2 and Viem
- Wallet connection (ConnectButton component with injected connector)
- Contract interaction utilities (useReadContract, useWriteContract)
...
- CI/CD pipeline
- Frontend tests
- Advanced Web3 features (ENS, contract events, multicall)
